...
// ── Schedules ─────────────────────────────────────────────────────────────
// ── Searchable flow picker ─────────────────────────────────────────────────
function FlowPicker({ value, onChange, graphs }) {
  ...
  function select(g) { setQuery(g.name); onChange(g.name); setOpen(false); }
  ...
  return (
    <div ref={wrapRef} style={{position:"relative"}}>
      <input required value={query} onChange={handleInput} onFocus={()=>setOpen(true)}
        placeholder="Search flows…" autoComplete="off" style={{width:"100%"}}/>
      {open && filtered.length > 0 && (
        <div style={{position:"absolute",top:"calc(100% + 4px)",left:0,right:0,background:"#1a1d2e",
          border:"1px solid #7c3aed",borderRadius:8,zIndex:999,maxHeight:220,overflowY:"auto",
          boxShadow:"0 8px 24px rgba(0,0,0,.5)"}}>
          {filtered.map(g=>(
            <div key={g.id} onMouseDown={()=>select(g)}
      ...
      <div className="card">
        ...
        <form onSubmit={create}>
          <div className="form-row">
            ...
            <div className="form-group">
              <label>Flow</label>
              <FlowPicker value={form.workflow} graphs={graphs} onChange={v=>setForm({...form,workflow:v})}/>
            </div>
          </div>
          <div className="form-row">
