...
**`bootstrap.sh` generates a unique `SECRET_KEY` automatically** and writes it into `.env` — no manual step required for new installs.
...
- Set `SECRET_KEY` in `.env` before storing any real credentials (auto-generated by `bootstrap.sh` for new installs).
