...
| UX2a | Global search (`Ctrl+K`) — `CommandPalette.jsx` overlay; searches pages, flows, runs, credentials; prefetches flows/credentials once per open and filters locally; stale-request guards prevent older responses from overwriting newer results; match highlight; ↑↓/↵/Esc navigation; Search button in sidebar footer |
...
| UX4 | Workspace switching + admin resilience polish — admin workspace switches no longer force a full reload: `WorkspaceContext.jsx` returns the switched workspace, `AdminLayout.jsx` handles async switch/toast/sidebar close, `App.jsx` remounts route trees per workspace, and `Workspaces.jsx` uses the shared switch flow; `CanvasApp.jsx` now confirms before discarding unsaved edits, clears stale graph state on workspace change, and reloads graphs + credentials for the selected workspace |
| Ops1 | Setup/env recovery + review stack parity — `setup.sh` now writes prompts to `/dev/tty`, appends missing `.env` keys, and safely reuses/cleans existing values; `app/core/secrets.py` normalises older prompt-polluted env values at startup; `tests/test_secrets.py` covers the recovery path; `docker-compose.yml` now parameterises Caddy host ports and `docker-compose.review.env` exposes the stack on `9999` without bypassing Caddy |
| UX5 | Admin safety pass — `AuditLog.jsx` tolerates malformed `detail` payloads instead of crashing, `Schedules.jsx` validates payload JSON before submit and fixes keyed edit-row rendering, `FlowPicker.jsx` guards against missing names, and `Users.jsx` now uses a focus-trapped reset-password modal with explicit loading/error states |
...
| S5 | Admin/canvas reliability follow-up — `useFocusTrap` now respects callback updates; `CommandPalette.jsx`, `ExtractSubflowModal.jsx`, `Credentials.jsx`, `Dashboard.jsx`, `Logs.jsx`, `Metrics.jsx`, `Schedules.jsx`, `Settings.jsx`, and `System.jsx` all got cleanup for focus trapping, keyed row rendering, quieter background refreshes, and awaited post-action reloads so the UI does not drift into stale or misleading states after API calls |
| T2 | Template quick-start + schedules query fix — the canvas welcome-state starter buttons now import real built-in templates directly (`email_to_slack`, `webhook_to_notion`, `sheets_to_slack_report`, `csv_to_db`) instead of just reopening the flow browser; `list_schedules()` now keeps `LEFT JOIN LATERAL` ahead of the workspace `WHERE` clause to avoid PostgreSQL syntax errors; `tests/test_workspace_scoping.py` covers the SQL ordering regression |
| S6 | Admin/canvas/auth reliability sweep — `Workspaces.jsx`, `Flows.jsx`, `Templates.jsx`, `Scripts.jsx`, `Users.jsx`, `HistoryModal.jsx`, `PermissionsModal.jsx`, and `CanvasApp.jsx` now clear stale state more aggressively on failed refreshes, await post-mutation reloads more consistently, and expose explicit error states; `AuthContext.jsx` also clears `currentUser` when `/api/auth/me` fails so expired sessions do not leave stale privileged UI visible |
| S7 | Admin diagnostics/loading hardening — `Dashboard.jsx`, `Logs.jsx`, and `Settings.jsx` now preserve known-good data during silent refresh failures, show explicit first-load error states, and use `finally` for maintenance/token busy flags so controls do not get stuck after failed actions |
...
**Rule:** After each sprint, update `AGENTS.md`: tick the item, add a row to
